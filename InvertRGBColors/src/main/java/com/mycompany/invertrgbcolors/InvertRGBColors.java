...
public class InvertRGBColors{
    public static void main(String[] args){
        ...
        System.out.printf("The Array Of R(Red) G(Green) B(Blue) Colors Is : %s%n", Arrays.toString(ArrayOfRGBColors));
...
//        System.out.printf("The Inverted Array Of R(Red) G(Green) B(Blue) Colors Is : %s%n", Arrays.toString(InvertedArrayOfRGBColors));
          int[] InvertedArrayOfRGBColors = new int[ArrayOfRGBColors.length];
          for (int j = 0 ; j < InvertedArrayOfRGBColors.length ; j++){
              InvertedArrayOfRGBColors[j] = 255 - ArrayOfRGBColors[j];
          }
          System.out.printf("The Inverted Array Of R(Red) , G(Green) , And Finally B(Blue) Colors Is : %s%n", Arrays.toString(InvertedArrayOfRGBColors));
    }
}